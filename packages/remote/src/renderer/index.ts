...
const sharedValueLocal = mitt<{
  changed: [name: string, value: Ref]
}>()
export const useShareValue = <T extends keyof SharedValueType, VT = SharedValueType[T]>(name: T) => {
  ...
  const handleLocalSync = ([name, value]: [name: string, value: Ref]) => {
  ...
  const watcher = watch(refValue, refValue => {
